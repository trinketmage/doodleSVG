;(function(root, factory) {
  ...
  doodleSVG.prototype.normalizeArray = function(a, b, c, d) {
    return (c = parseFloat(c) - parseFloat(a)), (d =
      parseFloat(d) - parseFloat(b)), Math.sqrt(c * c + d * d)
  }
  ...
  doodleSVG.prototype.normalize = function(el) {
    ...
    var pathLength,
      ...
      k,
      l,
      check = el.tagName.toLowerCase()

    if ('path' === check) {
      ;(i = el.style.strokeDasharray), (el.style.strokeDasharray =
        'none'), (pathLength = el.getTotalLength() || 0)
      try {
        BBox = el.getBBox()
      } catch (m) {}
      ...
      for (
        points = el
          ...
          .split(', ')
          .join(',')
          .split(' '), pathLength = 0, i = points[0].split(','), '' ===
          points[points.length - 1] && points.pop(), 'polygon' === check &&
          (
        ...
        ;(normPoints = points[j].split(',')), 1 === normPoints.length &&
          (normPoints[1] = points[j++]), 2 === normPoints.length &&
          (
            (pathLength +=
